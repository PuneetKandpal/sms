...
import AIOAnswersTableComponent from "./AIOAnswersTableComponent";
...
const AIOAnswerContentModal = ({ open, handleClose, answer }) => {
  if (!answer) return null;

  const handleCopyMarkdown = () => {
    navigator.clipboard.writeText(answer.generated_content || "");
    toast.success("Markdown copied to clipboard");
  };

  const handleCopyPlainText = () => {
    const plainText = (answer.generated_content || "")
      .replace(/<[^>]*>/g, "")
      .replace(/!\[.*\]\(.*\)/g, "")
      .replace(/\[(.*?)\]\(.*?\)/g, "$1")
      .replace(/[#*_`~>]/g, "")
          ...
          <div className="max-w-4xl mx-auto px-8 py-8">
            <div className="prose prose-lg prose-gray max-w-none">
              ...
              <ReactMarkdown remarkPlugins={[remarkGfm]}>
                {answer.generated_content || "No content available"}
              </ReactMarkdown>
            </div>
          </div>
        </div>
...
// Array cell components similar to ArticlesTable
...
export const OverviewCell = ({ text }) => {
  return (
    ...
    >
      <Tooltip title={text || "No question available"} placement="top" arrow>
...
const normalizeValue = (value, fallback = "draft") => {
...
const formatLabel = (value, fallback = "Draft") => {
  if (value === null || value === undefined) return fallback;
  const cleaned = value
    .toString()
    .trim()
...
const getStatusLabel = (status) => {
  return formatLabel(status, "Draft");
};
...
const getStageLabel = (stage) => {
  ...
  switch (normalizedStage) {
    ...
    case "rejected":
      return "Rejected";
    default:
      return formatLabel(stage, "Draft");
  }
};
...
export default function AIOAnswersTable({
  answerData = [],
  loading,
  onSelectionChange = () => {},
  selectedAnswerRows = [],
  ...
  const handleActionClick = (answer) => {
    if (normalizeValue(answer.status) !== "completed") {
      toast.error("Answer is not yet generated. Please wait for sometime.");
      return;
    }
  ...
  const handleOpenContentArchitecture = (answer) => {
    if (!projectId) {
    ...
    if (!answer?.page_id) {
      toast.error(
        "This answer is not linked to a Content Architecture page yet."
      );
      return;
    }

    router.push(
      `/projects/${projectId}/content-architecture?caTab=overview&pageId=${answer.page_id}&autoScroll=true`
    );
  };
    ...
    startTransition(() => {
      ...
      const result = answerData
        ...
        .filter((row) => {
          ...
          const rowStage = normalizeValue(row.stage);
          return filters.stage.includes(rowStage);
        });

      setFilteredData(result);
    });
  }, [answerData, debouncedSearchTerm, filters]);
  ...
  const sortedData = useMemo(() => {
    return [...filteredData].sort((a, b) => {
      ...
      if (sortConfig.key === "status") {
        aValue = statusOrder[normalizeValue(a.status)] || 0;
        bValue = statusOrder[normalizeValue(b.status)] || 0;
      } else if (sortConfig.key === "stage") {
        aValue = stageOrder[normalizeValue(a.stage)] || 0;
        bValue = stageOrder[normalizeValue(b.stage)] || 0;
  ...
  const selectedRowIds = useMemo(
    () => new Set(selectedAnswerRows.map((row) => row.id)),
    [selectedAnswerRows]
  );
  ...
  const handleCheckboxToggle = (row) => {
    const newSelection = [...selectedAnswerRows];
  ...
  return (
    <Box sx={{ backgroundColor: "#FAFAFA", width: "100%", p: 1 }}>
      ...
      {loading ? (
        <AIOAnswersTableSkeleton
          columns={columns}
          visibleColumns={visibleColumns}
          rowsCount={10}
        />
      ) : (
        <AIOAnswersTableComponent
          columns={columns}
          ...
          onAutoScrollSettled={onAutoScrollSettled}
        />
      )}
...
/**
 * AIOAnswersTableSkeleton renders a shimmer UI skeleton for the AIOAnswersTable component.
 */
function AIOAnswersTableSkeleton({ columns, visibleColumns, rowsCount = 10 }) {
  return (
